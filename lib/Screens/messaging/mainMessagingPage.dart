...
class _MainMessagingPageState extends State<MainMessagingPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      key: _scaffoldKey,
      body: Container(
        child: messagesListChecked ? messagesList.length==0 ? Center(
          child: Text('No Messages', style: TextStyle(fontWeight: FontWeight.w300, color: Colors.grey[500], fontSize: 25.0, letterSpacing: 1.0),),
        ) : Container(
          color: Colors.white,
                    ...
                    return StreamBuilder(
                      ...
                      builder: (context, snapshot2) {
                        if(snapshot2.data==null){
                          ...
                          String checkString = lMessage.message.split(":")[0];
                          ...
                          return Material(
                            child: InkWell(
                              child: ListTile(
                                ...
                                subtitle: Text(check? "File" : lMessage.message, style: TextStyle(fontSize: 14.0, fontWeight: FontWeight.w300, letterSpacing: 0.2),),
                                leading: selectedConvoIndex==index ? IconButton(
                                  icon: Icon(Icons.close, color: Colors.grey[800],),
                                  onPressed: (){
                                    setState(() {
                                      deleteWaiting = false;
                                      selectedConvoIndex = -1;
                                    });
                                  },
                                ) : CircleAvatar(
                                  child: Container(),
                                ...
                                trailing: deleteWaiting ? Container(
                                  width: 25.0,
                                  height: 25.0,
                                  child: Center(
                                    child: CircularProgressIndicator(
                                ...
                                ) : selectedConvoIndex==index ? IconButton(
...
//                                    _scaffoldKey.currentState.showSnackBar(snackBarRegistered);
                                    ...
                                    await _databaseService.deleteConversation(messagesList[index]);
                                  ...
                                  children: [
                                    ...
                                    snapshot.data['unreadMessage']==0 ? Container(height: 10.0,width: 10.0,) : lMessage.from==me.uid ? Container(height: 10.0,width: 10.0,) : Text('${snapshot.data['unreadMessage']}', style: TextStyle(color: Colors.redAccent, fontSize: 18.0, fontWeight: FontWeight.w600),),
                                  ],
                                ),
                              ),
                              onTap: (){
